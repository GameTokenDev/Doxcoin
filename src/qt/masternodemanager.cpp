...
#include "addeditdoxmasternode.h"
    ...
    walletModel(0)
{
    ...
    LOCK(cs_doxmaster);
    BOOST_FOREACH(PAIRTYPE(std::string, CDoxmasterNodeConfig) doxmaster, pwalletMain->mapMyDoxmasterNodes)
    {
        updateDoxmasterNode(QString::fromStdString(doxmaster.second.sAlias), QString::fromStdString(doxmaster.second.sAddress), QString::fromStdString(doxmaster.second.sMasternodePrivKey), QString::fromStdString(doxmaster.second.sCollateralAddress));
    }
...
static void NotifyDoxmasterNodeUpdated(MasternodeManager *page, CDoxmasterNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updateDoxmasterNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifyDoxmasterNodeChanged.connect(boost::bind(&NotifyDoxmasterNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifyDoxmasterNodeChanged.disconnect(boost::bind(&NotifyDoxmasterNodeUpdated, this, _1));
}
...
void MasternodeManager::updateDoxmasterNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_doxmaster);
    bool bFound = false;
    int nodeRow = 0;
...
{
    AddEditDoxmasterNode* aenode = new AddEditDoxmasterNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CDoxmasterNodeConfig c = pwalletMain->mapMyDoxmasterNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    DoxmasterNodeConfigDialog* d = new DoxmasterNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete Doxmaster Node?", "Are you sure you want to delete this doxmaster node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMyDoxmasterNodes.erase(sAddress);
        walletdb.EraseDoxmasterNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("Doxmaster Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("Doxmaster Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CDoxmasterNodeConfig) doxmaster, pwalletMain->mapMyDoxmasterNodes)
    {
        CDoxmasterNodeConfig c = doxmaster.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
