...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("dox://"))
    {
        uri.replace(0, 12, "dox:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "dox.lnk";
}
...
{
    return GetAutostartDir() / "dox.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("dox-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  dox-qt [" + tr("command-line options") + "]                     " + "\n";
