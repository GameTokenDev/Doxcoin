...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #doxcoinTEST\r");
            Send(hSocket, "WHO #doxcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #doxcoin\n");
            // randomly join #doxcoin00-#doxcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #doxcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #doxcoin%02d\r", channel_number).c_str());
        };
